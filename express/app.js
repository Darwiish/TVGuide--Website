...
app.get("/api/filteredprograms", (req, res) => {
    Program.find({ program_channel: req.query.chan},(err, Program) => {
        res.json(Program);
    });
});
...
app.post('/api/registerprogram', Authentication, function(req, res) {
    ...
    const {  program_name, firstDate, nextDate} = req.body;
    const program = new Job({ program_name,firstDate, nextDate, program_channel });
